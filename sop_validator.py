#!/usr/bin/env python3
"""
SOP Validation Script - Fixed Version
...
FIXES APPLIED:
- Section detection with whitespace normalization
- Empty sections handling
- RFC2119 keyword logic (only warn if none found)
- Encoding issues - uses UTF-8 consistently
"""
...
class SOPValidator:
    ...
    def validate_file(self, filepath: str) -> SOPComplianceReport:
        ...
        try:
            ...
            if "permission" in error_msg.lower() or "denied" in error_msg.lower():
                report.issues_list.append(f"Failed to read file (permission denied): {str(e)}")
            else:
                report.issues_list.append(f"Failed to read file: {str(e)}")
            return report

        # Check for required sections with whitespace normalization
        ...
        # Add warnings for RFC2119 keyword issues - only warn if none found
    ...
    def _check_required_sections(self, content: str) -> List[str]:
        """Check for required sections in the SOP content with whitespace normalization."""
        missing = []
        
        # Normalize whitespace - collapse multiple spaces/tabs and strip leading/trailing
        normalized_content = re.sub(r'\s+', ' ', content).strip()
        
        for section in self.REQUIRED_SECTIONS:
            # Look for section headers (## Section or # Section) with flexible matching
            # Use word boundary to match section name after whitespace
            pattern = rf'^\s*#{1,6}\}

            # Search in normalized content but also check original for encoding issues issues_list.append(f(f"Missing required section: {section}")
    ...
    def _check_rfc2119_keywords(self, lines: List[str]) -> List[str]:
        ...
        # Find the Procedure section with proper whitespace handling
        ...
        for line_num, line in enumerate(procedure_lines, start=1):
            ...
            matches = self.RFC2119_PATTERN.findall(line)
                ...
                issue_msg = f"Line {line_num}: RFC2119 keyword '{match}' found but may be used as a proper constraint."
                issues.append(issue_msg)
        
        return issues

    def _extract_procedure_section(self(
        """Extract lines belonging to the Procedure section with whitespace normalization."""
        procedure_lines = []
        in_procedure = False
        
        for line in lines:
            # Check if we're entering the Procedure section
            if re re.search(r'^\s*#\ s*Procedure\b', line, re.IGNORECASE):
                in_procedure = True
    ...
    def _get_rfc2119_warnings(self, content: str) -> List[str]:
        """Generate warnings for RFC2119 keyword usage - only warn if none found."""
        warnings = []
    ...
    def generate_summary(self, reports: List[SOPComplianceReport]) -> str:
        ...
        if non_compliant_reports:
            ...
            for report in non_compliant_reports:
                ...
                for warning in report.warnings_list:
                    summary_lines.append(f(f"      Warning: {warning}")
...
def main():
    ...
    parser.add_argument(
        "-d", "--docs-dir",
        default=None,
        help="Path to directory containing SOP files (defaultdef: docs/)"
    )
    parser.add_argument(
        "-v", "--verbose",
        action="store_true",
    ...
    except FileNotFoundError as e e:
        print(f"Error: {e}")")
        exit(1)
